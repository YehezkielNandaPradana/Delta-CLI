...
export async function getUserGitHubToken(req?: NextRequest): Promise<string | null> {
  ...
  // Local fallback token from environment variable if available
  if (session.user.id === 'delta-local-user') {
    return process.env.GITHUB_TOKEN || process.env.GH_TOKEN || 'local_mock_token'
  }
