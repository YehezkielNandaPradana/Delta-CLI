...
export async function executeCodexInSandbox(
  ...
  try {
    ...
    const isLocalRouter =
      Boolean(process.env.ROUTER9_BASE_URL) ||
      selectedModel === 'AntigravityCombo' ||
      selectedModel === 'KiloCombo' ||
      selectedModel === 'Delta' ||
      Boolean(selectedModel?.startsWith('ag/')) ||
      Boolean(selectedModel?.startsWith('openrouter/'))

    if (!isLocalRouter && !process.env.AI_GATEWAY_API_KEY) {
      return {
        success: false,
        error: 'AI Gateway API key not found. Please set AI_GATEWAY_API_KEY environment variable or use 9Router.',
        cliName: 'codex',
        changesDetected: false,
      }
    }
    ...
    if (!isLocalRouter && (!apiKey || (!isOpenAIKey && !isVercelKey))) {
    ...
    const is9Router =
      ...
      (process.env.ROUTER9_BASE_URL ||
        ...
        selectedModel === 'KiloCombo' ||
        selectedModel === 'Delta' ||
        selectedModel?.startsWith('ag/') ||
        selectedModel?.startsWith('openrouter/'))

    if (is9Router) {
      const routerBase = process.env.ROUTER9_BASE_URL || 'http://localhost:20128/v1'
