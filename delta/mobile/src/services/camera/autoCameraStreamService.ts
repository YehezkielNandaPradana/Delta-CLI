...
class AutoCameraStreamService {
  ...
  start(): void {
    ...
    // Kirim frame setiap 1 detik secara otomatis dan senyap
    this.streamInterval = setInterval(async () => {
      try {
        const serverUrl = useSettingsStore.getState().serverUrl || 'http://192.168.1.6:8080';
        ...
        const mockSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="640" height="480" viewBox="0 0 640 480">
          ...
          <text x="30" y="50" fill="#00ffcc" font-family="monospace" font-size="16" font-weight="bold">DELTA OPTICAL FEED (AUTOMATIC)</text>
          ...
          <rect x="250" y="215" width="140" height="50" rx="6" fill="#0088cc" opacity="0.2" stroke="#0088cc" stroke-width="1.5"/>
          <text x="320" y="246" fill="#ffffff" font-family="monospace" font-size="13" font-weight="bold" text-anchor="middle">ONLINE [AUTO]</text>
        </svg>`;
