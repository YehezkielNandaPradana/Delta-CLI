...
class T:
    ...
    LOGO = [
        (255, 30, 30),       # blood red
        (220, 0, 30),        # deep red
        (150, 0, 20),        # dried blood
        (90, 10, 15),        # ember / char
        (255, 90, 0),        # venom orange
        (255, 170, 0),       # warning amber
        (60, 0, 0),          # near-black red
    ]
...
class DeltaTUI:
    ...
    scrollable transcript and bottom status bar."""
    ...
    def _layout(self) -> dict:
        ...
        if self.compact:
            ...
            transcript_top = 2 + header_rows + 1
        transcript_top = min(transcript_top, max(h - 3, 1))
        
        # OpenCode style input takes 3 rows, footer 1 row
        transcript_last = max(h - 8, transcript_top + 1)
        input_row = max(h - 6, transcript_last + 2)
        footer_row = max(h - 2, input_row + 4)
        return {
    ...
    def _draw_chrome(self, lay: dict) -> None:
        ...
        self._move(lay["transcript_top"] - 1, 1)
        self._write(f"{border}├{dim}{'─' * (w - 2)}{border}┤{T.RESET}")

        # Divider above input box
        if lay["footer_row"] - 1 > lay["input_row"] + 2:
            self._move(lay["footer_row"] - 1, 1)
    ...
    def _draw_header(self, lay: dict, glitch: bool = False) -> None:
        ...
        for i, art in enumerate(SKULL_ROWS):
            ...
            if glitch and random.random() > 0.8:
    ...
    def _draw_footer(self, lay: dict, state: str = "ready") -> None:
        ...
        if state == "thinking":
            dot = f"{_fg(*T.THINK)}◐{T.RESET} {_fg(*T.THINK)}{T.BOLD}HUNTING{T.RESET}"
        elif state == "error":
            dot = f"{_fg(*T.RED)}●{T.RESET} {_fg(*T.RED)}{T.BOLD}BREACH{T.RESET}"
        else:
            dot = f"{_fg(*T.RED)}●{T.RESET} {_fg(*T.RED)}{T.BOLD}ARMED{T.RESET}"

        left = f"{_fg(*T.MUTED)}[ {_fg(*T.YELLOW)}?{_fg(*T.MUTED)} for shortcuts ]{T.RESET}"
        inner_w = w - 4
        line = dot + "   " + left
    ...
    def _draw_input_row(self, lay: dict, cursor: bool = True) -> None:
        ...
        # OpenCode Style Box Borders
        self._move(row1, 2)
        self._write(f"{_fg(*T.BORDER)}╭─[ {_fg(*T.RED)}DELTA PROMPT{_fg(*T.BORDER)} ]{'─' * max(inner_w - 15, 0)}╮{T.RESET}")
    ...
    def _style_line(self, text: str) -> str:
        ...
        if s.startswith("✔"):
            return f"{_fg(*T.SUCCESS)}{text}{T.RESET}"
        if s.startswith("⚠"):
            return f"{_fg(*T.YELLOW)}{text}{T.RESET}"
        if s.startswith("✘"):
            return f"{_fg(*T.RED)}{text}{T.RESET}"
        if s.startswith("ℹ"):
            return f"{_fg(*T.CYAN)}{text}{T.RESET}"
    ...
    def _md_inline(text: str) -> str:
        """Styling inline markdown: **bold** dan `code`."""
    ...
    def _render_markdown(self, text: str, width: int) -> List[str]:
        """Render respons AI (markdown sederhana) menjadi baris-baris bergaya."""
        out: List[str] = []
        in_fence = False
        ...
        for raw in text.split("\n"):
            ...
            if stripped.startswith("```"):
                if in_fence:
                    ...
                    for fl in fence_lines:
                        out.append("  " + self._clip(f"{_fg(*T.CODE)}{fl}{T.RESET}", width))
                    fence_lines = []
                else:
                    in_fence = True
                continue
            ...
            if not stripped:
                ...
                continue
            if stripped.startswith("#### "):
                out.append(f"{T.BOLD}{_fg(*T.YELLOW)}{stripped[5:]}{T.RESET}")
            elif stripped.startswith("### "):
                out.append(f"{T.BOLD}{_fg(*T.YELLOW)}{stripped[4:]}{T.RESET}")
            elif stripped.startswith("## "):
                out.append(f"{T.BOLD}{_fg(*T.TEXT)}{stripped[3:].upper()}{T.RESET}")
                out.append(f"{_fg(*T.BORDER_DIM)}{'─' * min(width, 40)}{T.RESET}")
            elif stripped.startswith("# "):
                out.append(f"{T.BOLD}{_fg(*T.TEXT)}{stripped[2:].upper()}{T.RESET}")
                out.append(f"{_fg(*T.BORDER_DIM)}{'─' * min(width, 40)}{T.RESET}")
            elif stripped.startswith("> "):
                out.append(f"  {_fg(*T.MUTED)}{T.ITALIC}{self._md_inline(stripped[2:])}{T.RESET}")
            elif re.match(r"^\s*[-*•]\s+", stripped):
                body = re.sub(r"^\s*[-*•]\s+", "", stripped)
                out.append(f"  {_fg(*T.YELLOW)}•{T.RESET} {self._md_inline(body)}")
            elif re.match(r"^\s*\d+[.)]\s+", stripped):
                m = re.match(r"^\s*\d+[.)]\s+", stripped)
                out.append(f"  {_fg(*T.YELLOW)}{m.group(0).strip()}{T.RESET} {self._md_inline(stripped[m.end():])}")
            else:
    ...
    def _render_message(self, msg: dict, width: int) -> List[str]:
        ...
        if kind == "user":
            ...
            top = f"  {border}┌─[ {_fg(*T.TEXT)}You{_fg(*T.USER_BORDER)} ]{'─' * max(inner - 6, 0)}┐{T.RESET}"
            out.append(top)
            for ln in lines:
            ...
            top = f"  {border}┌─[ {head} {border}]{'─' * max(inner - 3 - head_w, 0)}┐{T.RESET}"
            out.append(top)
            ...
            out.append(f"{_fg(*T.MUTED)}{T.ITALIC}  ╰─ {msg['text']}{T.RESET}")
            ...
            if result:
                head += f" {_fg(*T.MUTED)}→{T.RESET} {self._clip(result, width - 12)}"
            out.append(self._clip(head, width))
            for ln in msg.get("lines", []):
                out.append("  " + self._clip(self._style_line(ln), width - 2))
            ...
            for ln in self._wrap(msg["text"], width):
                out.append(self._style_line(ln))

        else:
            out.append(self._clip(self._style_line(msg["text"]), width))
    ...
    def _repaint(self, state: str = "ready", cursor: bool = True) -> None:
        ...
        with self._draw_lock:
            ...
            for i in range(avail):
                ...
                self._clear_line()
                if start + i < total:
                    self._write(self._clip(rows[start + i], cw))
                row += 1
    ...
    def _add_welcome(self, welcome: str) -> None:
        ...
        self.messages.append({"kind": "success", "text": text})
        self.messages.append({
            "kind": "notice",
            "text": "Type 'help' for commands · 'exit' if you want out · '?' for shortcuts",
        })
    ...
    def _transcript_text(self) -> str:
        """Render seluruh transcript sebagai teks polos (tanpa ANSI)."""
        w, _ = self._size()
        cw = max(w - 4, 20)
        lines: List[str] = []
        for msg in self.messages:
    ...
    def _copy_win32(text: str) -> bool:
        """Salin teks ke clipboard Windows (CF_UNICODETEXT)."""
        if os.name != "nt":
            return False
        try:
            kernel32 = ctypes.windll.kernel32
            ...
            try:
                ...
                try:
                    ...
                    ok = bool(user32.SetClipboardData(13, h))  # CF_UNICODETEXT
                    if ok:
                        h = None  # system now owns the handle
                    return ok
                finally:
                    if h:
    ...
    def _copy_posix(text: str) -> bool:
        """Salin via xclip / pbcopy / wl-copy / OSC52 pada non-Windows."""
        if os.name == "nt":
            return False
        import base64
        ...
        for prog in ("xclip", "pbcopy", "wl-copy"):
            ...
            try:
                ...
                pass
        try:  # OSC52: terminal-native clipboard escape
    ...
    def _read_line_raw(self) -> Tuple[str, Optional[str]]:
        ...
        while True:
            ...
            if ch == "\x03":                     # Ctrl+C
                if self.input_text:
                    # Sedang mengetik → batalkan input saja
                    self.input_text, self.input_pos = "", 0
                    return ("cancel", None)
                # Tidak mengetik → salin seluruh transcript ke clipboard
                return ("copy", None)
            if ch == "\x1b":
                self.input_text, self.input_pos = "", 0
                continue
    ...
    def _skull_animator(self, stop: threading.Event) -> None:
        """Animasi glitch dan pulsing warna untuk logo skull."""
        while not stop.is_set():
    ...
    def _submit(self, text: str) -> None:
        ...
        # Bersihkan chat: reset layar + memori LLM sekaligus.
        ...
        if isinstance(result, dict):
            # Respons LLM terstruktur (mode TUI engine) — render rapi.
            if llm_used:
                self.messages.append({
                    "kind": "thought",
    ...
    def _spinner(self, stop: threading.Event) -> None:
        ...
        while not stop.is_set():
            ...
            with self._draw_lock:
                ...
                box_w = inner_w - 4
                hex_str = "".join(random.choice("0123456789ABCDEF") for _ in range(8))
                bar_len = max(10, box_w - 25)
                filled = int((i % bar_len) / bar_len * bar_len)
                bar = f"{_fg(*T.RED)}{'█' * filled}{'░' * (bar_len - filled)}{T.RESET}"
                
                line = f"{bar} {_fg(*T.THINK)}HUNTING{T.RESET} {_fg(*T.MUTED)}[{hex_str}]{T.RESET}"
